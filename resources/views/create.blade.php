...
@section('content')
<h2 class="text-center mb-4">Добавить работника</h2>
<div class="row">
    <div class="col-lg-6 mx-auto">
        ...
        <form method="POST" action="{{ route('workers.store') }}">
            ...
            <div class="form-group" title="Обязательно к заполнению">
                ...
                <input type="number" min="1000" name="salary" value="{{ old('salary') }}" 
                       class="form-control {{ $errors->has('salary') ? 'is-invalid' : '' }}" id="worker-salary">
                @if ($errors->has('salary'))
                    <div class="invalid-feedback">{{ $errors->first('salary') }}</div>
                @endif
            </div>
            <button type="submit" class="btn btn-dark mx-auto">Добавить работника</button>
        </form>
    </div>
</div>
@endsection